                                ...
                                @foreach($addresses as $address)
                                    <label class="block border-4 rounded-3xl p-8 cursor-pointer transition hover:shadow-xl {{ $address->is_primary ? 'border-indigo-600 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="address_id" value="{{ $address->id }}" 
                                               class="float-right mt-2 w-6 h-6 text-indigo-600" 
                                               {{ $address->is_primary ? 'checked' : '' }} required>
                                        <p class="font-bold text-2xl text-gray-800">{{ $address->recipient_name }} {{ $address->is_primary ? '(Alamat Utama)' : '' }}</p>
                                        <p class="text-xl text-gray-700 mt-3">{{ $address->phone }}</p>
                                        <p class="text-lg text-gray-600 mt-4">{{ $address->full_address }}, {{ $address->city }}, {{ $address->postal_code }}</p>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-3xl shadow-lg border border-gray-200 p-8">
                            ...
                            <div class="space-y-6">
                                <label class="block border-4 rounded-3xl p-8 cursor-pointer transition hover:shadow-xl border-gray-200">
                                    <input type="radio" name="payment_method" value="transfer" class="float-right mt-2 w-6 h-6 text-indigo-600" required>
                                    <p class="font-bold text-2xl text-gray-800">Transfer Bank</p>
                                    <p class="text-xl text-gray-700 mt-4">BCA a.n bayu - 1234567890</p>
                                ...
                                <label class="block border-4 rounded-3xl p-8 cursor-pointer transition hover:shadow-xl border-indigo-600 bg-indigo-50">
                                    <input type="radio" name="payment_method" value="qris" class="float-right mt-2 w-6 h-6 text-indigo-600" checked required>
                                    ...
                                    <div class="mt-6 text-center">
                                        ...
                                        <p class="text-lg text-gray-600 mt-4">Scan QRIS di atas dengan aplikasi pembayaran</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-3xl shadow-lg border border-gray-200 p-8 sticky top-24">
                            ...
                            <div class="mt-8">
                                ...
                                <!-- TOMBOl BAYAR SEKARANG (BISA DIKLIK!) -->
